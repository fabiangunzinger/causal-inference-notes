...
The key insight of CUPED is that, given the right data, it is possible to transform the raw outcome metric in a way that leaves the treatment effect estimator ($\tee$) unbiased while reducing its variance. @deng2013improving motivate the approach using an insight from the Monte Carlo simulation literature. In the following, I want to spell out their approach step by step.
...
We can then transform $\yoi$ for each unit such that

$$
\y^{cuped}_i = \yoi - \theta\bar{X} + \theta\mathbb{E}[X_i].
$$
