...
We have seen in chapter @sec-power that if we want to control our Type I and Type II error rate, the required total sample size our experiment is given by:

$$
N = \frac{(z_{1 - \beta} + z_{\alpha/2})^2}{P(1-P)}\left(\frac{\sev}{\te}\right)^2.
$$

Now, $\za$ and $\zb$ are fixed given ...


This makes clear that reducing $\sev$ is one of our main levers to reduce required sample size.



## CUPED

CUPED stands for "**C**ontrolled experiments **U**sing **P**re-**E**xperiment **D**ata", and reduces variance by partialling out outcome metric variance based on pre-experiment data.

Introduced by researchers at Microsoft in 2013, it has since become an industry standard and is used by companies such as Booking.com, Netflix, and Meta, as well as commercial experimentation platforms such as Eppo and Statsig.


### How does CUPED work

The key insight of CUPED is that, given the right data, it is possible to transform the raw outcome metric in a way that leaves the treatment effect estimator ($\tee$) unbiased while reducing its variance. @deng2013improving motivate the approach as follows using an insight from the Monte Carlo simulation literature. Spelling out clearly the logic of their paper we get the following:

Assume that, in addition to our outcome variable $\yio$, we have access to another random variable, $X_i$ with known expectation $\mathbb{E}[X_i]$.

We can then transform $\yio$ for each unit such that

$$
\yio^{cuped} = \yio - \theta\bar{X} + \theta\mathbb{E}[X_i].
$$

If we do this for all units in our experiment, we will get
