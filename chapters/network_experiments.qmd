...
- Network experiments are experiments in which the outcome of some units is affected by the treatment status of other units.

- This violates [SUTVA](neyman_rubin_causal_model.qmd## Stable unit treatment value assumption (SUTVA)).
...
- Fundamentally, this happens because with interference, the ATE is no longer an unbiased estimate of the difference between a world where everyone is treated and nobody is treated, which is what standard ATE estimates are estimating.
...
- Marketplace interference: there are at least two types of units that form an online marketplace (i.e. sellers and buyers), and interference often happens when units compete for finite resources. Examples are ride hailing (customers competing for a finite number of rides), job matching (job hunters competing for a finite number of jobs), ads marketplaces (marketers competing for a fixed number of adds), education platforms, and food delivery (customers and restaurants competing for a fixed number of riders), which is an example of a three-sided marketplace. 
